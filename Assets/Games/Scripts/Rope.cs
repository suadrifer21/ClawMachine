...
public class Rope : MonoBehaviour
{
    ...
    private int minRopeLength;
    [SerializeField] private int maxRopeLength = 30;
    ...
    [SerializeField] private float ropedelay = 2f;

    public delegate void OnClawGrabChangeDelegate(bool isGrabbing);
    public event OnClawGrabChangeDelegate OnClawGrabChange;
    ...
    private IEnumerator DoStretch()
    {
        float elapsedTime = 0;
        int curLength = ropeLength;
        float lerp = 0;
        while (elapsedTime < ropedelay)
        {
            lerp += elapsedTime / ropedelay;
            ropeLength = (int)Mathf.Lerp(curLength, maxRopeLength, lerp);
            elapsedTime += Time.deltaTime;
            yield return null;
        }
        OnClawGrabChange?.Invoke(true);
    }
    private IEnumerator DoRetract()
    {
        ...
        {
            lerp += elapsedTime / ropedelay;
            ropeLength = (int)Mathf.Lerp(curLength, minRopeLength, lerp);
            elapsedTime += Time.deltaTime;
            yield return null;
        }
        OnClawGrabChange?.Invoke(false);
    }
}
